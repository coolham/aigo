...
def test2():
    face_rec = face_recognition()  # 创建对象
    person_data1 = loadPersonData(face_rec, 'jobs')  # 创建一个类保存相关信息，后面还要跟上人名，程序会在data文件中查找对应npy文件，比如这里就是'jobs.npy'
    person_data2 = loadPersonData(face_rec, 'jobs2')  # 导入第二张图片
    comparePersonData(person_data1, person_data2)  # 计算欧式距离，判断是否是同一个人
